    ...
    // chara
    {
        ...
        if (G.bezT < 1) {
            ...
            for (let i = 0; i < G.bez.length; i++) {
                let p = G.bez[i]
                ch.velocity.x += calc_bez(p[0]*p[1], G.bezT, G.bez.length, i)
                ch.velocity.y += calc_bez(p[0]*p[2], G.bezT, G.bez.length, i)
            }
        ...
        ch.pos.x += ch.velocity.x
        ch.pos.y += ch.velocity.y
...
function mousedown (ev) {
    ...
    let n = 5
    for (let i = 0; i < n; i++) {
        G.bez.push([Math.random() * 0.02, rand_width(0.1), rand_height(0.1)])
    }
}
